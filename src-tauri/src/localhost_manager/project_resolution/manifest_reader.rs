...
fn parse_toml_manifest(content: &str, target_section: &str) -> (Option<String>, HashSet<String>) {
    ...
    for signature in known_dependency_signatures() {
        if lower.contains(signature) {
            dependencies.insert((*signature).to_string());
        }
    }
    (name, dependencies)
}
...
fn parse_csproj(content: &str) -> (Option<String>, HashSet<String>) {
    ...
    let lower = content.to_ascii_lowercase();
    let dependencies = known_dependency_signatures()
        .iter()
...
pub(crate) fn infer_kind(command: &str, dependencies: &[String]) -> DevelopmentServerKind {
    ...
    if contains_any(
        ...
        &values,
        &["next", "nuxt", "@remix-run/node", "@sveltejs/kit"],
    ) {
...
fn contains_any(value: &str, signatures: &[&str]) -> bool {
    signatures.iter().any(|signature| value.contains(signature))
}
fn contains_any_slice(values: &[&str], signatures: &[&str]) -> bool {
    signatures.iter().any(|signature| {
        values
            .iter()
            .any(|value| value.eq_ignore_ascii_case(signature))
    })
}
fn known_dependency_signatures() -> &'static [&'static str] {
    &[
        "next",
        "nuxt",
        "@remix-run/node",
...
mod tests {
    use super::{
        infer_kind, parse_csproj, parse_go_module, parse_package_json, parse_toml_manifest,
    };
    use crate::localhost_manager::models::DevelopmentServerKind;
    ...
    fn parses_package_identity_and_framework() {
        let (name, dependencies) =
            parse_package_json(r#"{"name":"portal","devDependencies":{"vite":"1"}}"#).unwrap();
        assert_eq!(name.as_deref(), Some("portal"));
        assert_eq!(
            infer_kind("", &dependencies.into_iter().collect::<Vec<_>>()),
            DevelopmentServerKind::Frontend
        );
    }
    ...
    fn parses_other_allowlisted_manifest_names() {
        assert_eq!(
            parse_toml_manifest("[project]\nname = \"api\"", "project")
                .0
                .as_deref(),
            Some("api")
        );
        assert_eq!(
            parse_toml_manifest("[package]\nname = \"worker\"", "package")
                .0
                .as_deref(),
            Some("worker")
        );
        assert_eq!(
            parse_go_module("module github.com/acme/service\n").as_deref(),
            Some("service")
        );
